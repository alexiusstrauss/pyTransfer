...
from pytransfer.models import Pessoa, Token, Balance, History
from pytransfer.serializers import PessoaSerializer
...
class PyTestProject(APITestCase):
    ...
    def setUp(self):
        ...
        """
        response = self.client.post(self.url_pessoa, self.Alexius, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

        response = self.client.post(self.url_pessoa, self.Luis, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)

        response = self.client.post(self.url_pessoa, self.Fernando, format='json')
        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
    ...
    def test_verificar_se_tem_saldo_suficiente(self):
        ...
        payload = {
            ...
            "token_destino": token.codigo,
            "valor": (token.pessoa.balance + 5000),  # valor maior que saldo em conta
            "status": "Finalizado"
        }        
    ...
    def test_transferir_entre_duas_pessoas(self):
        ...
        payload = {
            "remetente": remetente.id,                  # primeira pessoa (remetente)
    ...
    def test_verificar_saldo_transferencia_status_pendente(self):
        ...
        self.assertEqual(remetente.balance, (remetente_saldo - valor_transacao))
    ...
    def test_verificar_saldo_transferencia_status_finalizado(self):
        ...
        self.assertEqual(destinatario.balance, (destinatario_saldo + valor_transacao))
